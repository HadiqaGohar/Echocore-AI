    ...
    <div className="flex flex-col items-center gap-3">
      <div className="relative">
        ...
        <motion.button
          ...
          className={`relative z-10 flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br ${config.color} shadow-2xl ${config.shadow} ring-4 ring-white/10 transition-all`}
        >
          {state === "idle" && <Mic className="h-9 w-9 text-white" />}
          {state === "recording" && (
            <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 0.8, repeat: Infinity }}>
              <MicOff className="h-9 w-9 text-white" />
            </motion.div>
          )}
          {state === "processing" && <Loader2 className="h-9 w-9 text-white animate-spin" />}
          {state === "speaking" && (
