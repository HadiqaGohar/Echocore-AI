  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="relative z-50 flex h-full flex-col overflow-hidden border-r border-white/10 bg-[#171717] shrink-0"
      >
        {/* Top actions */}
        <div className="flex items-center justify-between p-3">
          ...
          <button className="flex h-9 w-9 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-white/10 hover:text-white">
        ...
        <div className="px-3 pb-2">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-medium text-white transition-colors hover:bg-white/10"
          >
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-3">
          {Object.entries(grouped).map(([group, convs]) => (
            <div key={group} className="mb-2">
              <p className="px-2 pb-1 pt-3 text-xs font-semibold text-gray-500">{group}</p>
              {convs.map((conv) => (
                <div
                  key={conv.id}
                  onMouseEnter={() => setHoveredId(conv.id)}
                  onMouseLeave={() => setHoveredId(null)}
                  className="group relative flex items-center gap-3 rounded-lg px-2 py-2.5 text-sm text-gray-300 transition-colors hover:bg-white/5"
                >
                  ...
                  <AnimatePresence>
                    {hoveredId === conv.id && (
                      <motion.div
                        ...
                        className="absolute right-1 flex items-center gap-0.5 rounded-lg bg-[#2f2f2f] p-0.5 shadow-lg"
                      >
                        <button className="flex h-7 w-7 items-center justify-center rounded-md text-gray-400 hover:bg-white/10 hover:text-white">
        ...
        <div className="border-t border-white/10 p-3">
          <div className="flex items-center gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-white/5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-white truncate">Hadiqa Gohar</p>
